...
{
    saveInt (fd, lastTrace);
    for (int i = 0; i < MAXTRACES; ++i) {
        saveInt (fd, traces[i].srcX);
        saveInt (fd, traces[i].srcY);
        saveInt (fd, traces[i].destX);
        saveInt (fd, traces[i].destY);
...
{
    lastTrace = loadInt (fd);
    for (int i = 0; i < MAXTRACES; ++i) {
        traces[i].srcX = loadInt (fd);
        traces[i].srcY = loadInt (fd);
        traces[i].destX = loadInt (fd);
        traces[i].destY = loadInt (fd);
...
shCreature::loadState (int fd)
{
    ...
    switch (mMimic) {
    case kObject: mMimickedObject = &AllIlks[loadInt (fd)]; break;
    case kFeature: mMimickedFeature = (shFeature::Type) loadInt (fd); break;
    case kMonster: mMimickedMonster = &MonIlks[loadInt (fd)]; break;
    case kNothing:
    default:
        loadInt (fd); break;
    }
...
saveGame ()
{
    ...
    snprintf (savename, sizeof(savename)-1, "%s/%s.sav", UserDir, Hero.mName);
