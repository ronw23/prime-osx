...
shMapLevel::buildTown ()
{
    ...
    for (int x = 0; x < MAPMAXCOLUMNS; ++x)
        for (int y = 0; y < MAPMAXROWS; ++y) {
            ...
            if (v) for (int i = 0; i < v->count (); ++i)
                if (v->get (i)->isA (kObjGenericComputer)) {
                    computer = true;
                    goto done_searching_for_computer;
                }
        }
    done_searching_for_computer: ;
    if (!computer) { /* Oh, there was no computer randomly generated. */
        shObject *comp = pickFromCategory (kObjGenericComputer);
        comp->setUnpaid ();
    ...
    for (int i = 0; i < 3; ++i) {
        shMonster *guard = new shMonster (kMonGuardbot);
    ...
    for (int i = 0; i < 14; i++) {
        ...
        } while (!botilk or !IS_ROBOT (MonIlks[botilk].mType));
...
shMapLevel::buildRabbitLevel ()
{
    ...
    pills->mCount = MAINFRAMELEVELS;
    putObject (pills, 46, 10);

    shMonster *rabbit = new shMonster (kMonKillerRabbit);
