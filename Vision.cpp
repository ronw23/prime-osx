...
shMapLevel::computeVisibility ()
{
    ...
    for (x = Hero.mX;   x < MAPMAXCOLUMNS and !isOcclusive (x, Hero.mY);   x++)
        mVisibility[x][Hero.mY] = 1;
    for (x = Hero.mX;   x >= 0 and !isOcclusive (x, Hero.mY);   x--)
        mVisibility[x][Hero.mY] = 1;
    for (y = Hero.mY;   y < MAPMAXROWS and !isOcclusive (Hero.mX, y);   y++)
        mVisibility[Hero.mX][y] = 1;
    for (y = Hero.mY;   y >= 0 and !isOcclusive (Hero.mX, y);   y--)
        mVisibility[Hero.mX][y] = 1;
...
#define MAXD 100
    ...
    if (Hero.mY > 1) {
        ...
        for (x = Hero.mX, y = Hero.mY-1; x > 0 and !isOcclusive (x, y);
             x--)
        {
            mVisibility[x][y-1] = mVisibility[x-1][y-1] =
                mVisibility[x-1][y] = 1;
    ...
    if (Hero.mX > 1) {
        ...
        for (x = Hero.mX-1, y = Hero.mY; y > 0 and !isOcclusive (x, y);
             y--)
        {
            mVisibility[x-1][y] = mVisibility[x-1][y-1] =
                mVisibility[x][y-1] = 1;
        ...
        for (x = Hero.mX - radius; x <= Hero.mX + radius; x++) {
            for (y = Hero.mY - radius; y <= Hero.mY + radius; y++) {
                if (Level->isInBounds (x, y)
                    and distance (&Hero, x, y) < 5 * radius)
                {
                    mVisibility[x][y] = 1;
                }
            }
