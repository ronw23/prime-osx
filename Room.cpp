...
int
shMapLevel::makeTradePost (int sx, int sy, int ex, int ey)
{
    shMonster *trader;
    int dx = (sx + ex + RNG (0, 1)) / 2;
    int dy = (sy + ey + RNG (0, 1)) / 2;

    trader = new shMonster (kMonMelnorme);
    putCreature (trader, dx, dy);
    trader->mMelnorme.mSX = sx + 1; /* Do not include room walls. */
    trader->mMelnorme.mSY = sy + 1;
    trader->mMelnorme.mEX = ex - 1;
    trader->mMelnorme.mEY = ey - 1;
    trader->mMelnorme.mKnowledgeExhausted = 0;
    for (int i = 0; i < kMelnMaxService; ++i)
        trader->mMelnorme.mPermute[i] = i;
    shuffle (trader->mMelnorme.mPermute, kMelnMaxService, sizeof (int));
...
shMapLevel::addFeature (shFeature *f)
{
    mFeatures.add (f);
    if (f->isStairs ()) {
        mSquares[f->mX][f->mY].mFlags &= ~shSquare::kDark;
    }
}
