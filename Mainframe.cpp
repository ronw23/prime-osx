...
retry:
    ...
    //TODO: Populating a Mainframe should be handled by Map.cpp
    /* Add some monsters. */
    for (int i = 0; i < 22; ++i) {
        shMonId ilk;
        do {
            ilk = pickAMonsterIlk (i < 5 ? RNG (15, 20) : RNG (1, 15));
        } while (!ilk or !IS_PROGRAM (MonIlks[ilk].mType));
        shMonster *m = new shMonster (ilk);
        findUnoccupiedSquare (&x, &y);
        putCreature (m, x, y);
    } /* Put a red pill somewhere.  Might as well be under last monster. */
