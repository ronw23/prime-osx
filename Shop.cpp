...
shHero::leaveShop ()
{
    ...
    } else if (0 == v.count () and !shopkeeper->mShopKeeper.mBill) {
        if (tryToTranslate (shopkeeper)) {
            I->p ("\"Come again soon!\"");
        } else {
...
void
shHero::dropIntoOwnedVat (shObject *obj)
{
    int cnt = obj->mCount;
    int price = cnt * 10;
    shMonster *shopkeeper = mLevel->getShopKeeper (mX, mY);
...
void
shHero::quaffFromOwnedVat (shFeature *vat)
{
    const char* vatAds[] =
    {
...
void
shHero::billableOffense (shObject *obj, const char *gerund, int fee)
{
    if (obj->isUnpaid ()) {
        shMonster *shopkeeper = Hero.mLevel->getShopKeeper (Hero.mX, Hero.mY);
        if (shopkeeper) {
            if (tryToTranslate (shopkeeper)) {
                I->p ("%s testily beeps something about %d buckazoids",
                      THE (shopkeeper), fee);
...
void
shHero::employedItem (shObject *obj, int fee)
{
    if (obj->isUnpaid ()) {
        shMonster *shopkeeper = Hero.mLevel->getShopKeeper (Hero.mX, Hero.mY);
        if (shopkeeper) {
            if (!fee) {
                if (Hero.tryToTranslate (shopkeeper)) {
...
shHero::payShopkeeper ()
{
    ...
    shMonster *trader = mLevel->getMelnorme ();
    shObject *obj;
    shObjectVector v;
    int price;
    ...
    delete menu;
    Hero.resetStoryFlag ("theft warning");
    I->drawSideWin (); /* Monetary status might have changed. */
}
...
shMonster::doShopKeep ()
{
    ...
    while (-1 == res) {
        ...
        case kReady:
            ...
            if (canSee (&Hero) and
                mLevel->isInShop (Hero.mX, Hero.mY) and
                ( /* If hero is not in _our_ shop ignore it. */
                 mLevel->getRoomID (Hero.mX, Hero.mY) ==
                 mLevel->getRoomID (mX, mY)
                ))
            {
                if (mLevel->isInDoorWay (Hero.mX, Hero.mY)) {
                    shObjectVector v;
        ...
        if (m->isHostile ()) {
            ...
            if (I->yn ("Offer $%d to %s?", bribe, THE (m))) {
                if (countMoney () >= bribe) {
                    ...
                    loseMoney (bribe);
                    m->gainMoney (bribe);
                    m->mDisposition = shMonster::kIndifferent;
                } else {
