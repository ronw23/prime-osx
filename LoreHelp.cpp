...
findLoreEntry (const char *entry, int type)
{
...
static const char *
nameIntrinsic (int intr)
{
    switch (intr) {
    case kTelepathy:             return "telepathy";
    case kMotionDetection:       return "motion detection";
    case kTremorsense:           return "tremorsense";
    case kScent:                 return "acute smell sense";
    case kXRayVision:            return "x-ray vision";
    case kReflection:            return "reflection";
    case kAutoSearching:         return "automatic searching";
    case kTranslation:           return "language translation";
    case kHealthMonitoring:      return "health monitoring";
    case kRadiationProcessing:   return "radiation processing";
    case kSenseLife:             return "life sense";
    case kAutoRegeneration:      return "regeneration";
    case kLightSource:           return "light";
    case kPerilSensing:          return "sense of peril";
    case kBlind:                 return "blindness";
    case kShielded:              return "energy shield";
    case kBugSensing:            return "bug sensing";
    case kFlying:                return "flight";
    case kLucky:                 return "luck";
    case kNarcolepsy:            return "narcolepsy";
    case kBrainShielded:         return "brain shield";
    case kScary:                 return "scariness";
    case kAcidBlood:             return "acidic blood";
    case kMultiplier:            return "fertility";
    case kAirSupply:             return "air supply";
    case kBreathless:            return "breathlessness";
    case kCanSwim:               return "swimming";
    case kNightVision:           return "night vision";
...
void
analyzeObject (shMenu *lore, shObject *obj, int precisely)
{
    char *buf = GetBuf ();
    if (BOFH) { /* For debugging purposes. */
        const int props = 7;
        char status[props+1] = "-------";
        static int (shObject::*tests[props])(void) =
        {
            &shObject::isAppearanceKnown,
            &shObject::isBugginessKnown,
            &shObject::isChargeKnown,
            &shObject::isEnhancementKnown,
            &shObject::isInfectedKnown,
            &shObject::isCrackedKnown,
            &shObject::isKnown
        };
        for (int i = 0; i < props; ++i) {
            if ((obj->*tests[i]) ())  status[i] = '+';
        }
    ...
    if (hasAttacks and obj->isKnown ()) {
        pbrk = 1;
        /* Find first parent ilk that is abstract. */
        shObjId cat = ilk->mParent;
        while (cat != kObjNothing and AllIlks[cat].mProbability != ABSTRACT)
            cat = AllIlks[cat].mParent;
        snprintf (buf, SHBUFLEN, "Category: %s",
            cat == kObjNothing ? "uncategorized" : AllIlks[cat].mReal.mName);
        lore->addText (buf);
        lore->addText ("");
        ...
        for (int pass = 1; pass <= 4; ++pass) {
            ...
            case 1:
                if (ilk->mMeleeAttack) {
                    s = Hero.getSkill (ilk->mMeleeSkill);
                    if (s)
            ...
            case 3:
                if (ilk->mGunAttack) {
                    shSkill *s = Hero.getSkill (ilk->mGunSkill);
                    if (s)
                        snprintf (buf, SHBUFLEN, "Skill: %s", s->getName ());
                    else
    ...
    } else if (obj->isA (kArmor) and obj->isAppearanceKnown ()) {
        if (pbrk) lore->addPageBreak ();
        pbrk = 1;
        shObjectIlk *ilk = obj->myIlk ();
        if (obj->isEnhanceable ()) {
            shObjId cat = ilk->mParent;
            while (cat != kObjBioArmor and cat != kObjMBioArmor0 and
                   cat != kObjNothing and AllIlks[cat].mProbability != ABSTRACT)
                cat = AllIlks[cat].mParent;
            snprintf (buf, SHBUFLEN, "Armor subclass: %s   Max enhancement: %+d",
                AllIlks[cat].mReal.mName, ilk->mMaxEnhancement);
        } else {
            snprintf (buf, SHBUFLEN, "Armor subclass: %s   (not enhanceable)",
                AllIlks[ilk->mParent].mReal.mName);
        }
        lore->addText (buf);
        if (obj->isEnhancementKnown ()) {
        ...
        lore->addText (buf);
        if (ilk->mSpeedBoost and obj->isKnown ()) {
    ...
    if (obj->isAppearanceKnown () and obj->isA (kCanister)) {
    ...
    if (obj->isInfectable ()) {
        ...
        if (obj->isA (kObjGenericComputer)) {
            if (obj->isFooproof ()) {
                ...
                if (obj->isInfectedKnown () and obj->isInfected ()) {
            ...
            lore->addText (buf);
            if (!obj->isCrackedKnown ()) {
                lore->addText ("The software could be cracked.");
            } else if (obj->isCracked ()) {
    ...
    if (obj->isA (kCanister) and (!obj->isA (kObjLNO) or !obj->isKnown ())) {
        snprintf (buf, SHBUFLEN, "This object is vulnerable to %s.",
    ...
    if (obj->isA (kWeapon)) {
        ...
        snprintf (buf, SHBUFLEN, "Damage mod: %+d",
            obj->mEnhancement - 2 * obj->mDamage);
        lore->addText (buf);
    } else if (obj->isA (kArmor)) {
        shObjectIlk *ilk = obj->myIlk ();
        ...
        lore->addText ("");
        const shAbilities *a = obj->getAbilityModifiers ();
        if (a) {
            FOR_ALL_ABILITIES (i) {
                int mod = a->getByIndex (i);
                if (mod) {
...
void
shObject::showLore (int analyze)
{
    shMenu *lore = I->newMenu (inv (), shMenu::kNoPick);
    analyzeObject (lore, this, analyze);
    const char *objname = apparent ()->mName;
    const encyclopedia_entry *objlore = findLoreEntry (objname, 0);
    if (isA (kRayGun)) {
    ...
    if (!objlore and !isA (kRayGun)) {
        ...
        else if (isA (kObjGenericKeycard)) /* Skip fake lore. */;
    ...
    shObjectIlk *ilk = myIlk ();
    if (isAppearanceKnown ()) do {
        do {
            if (ilk->mParent) {
                ilk = &AllIlks[ilk->mParent];
                objlore = findLoreEntry (ilk->mReal.mName, 0);
            } else {
                objlore = NULL;
            }
        } while (!objlore and ilk->mProbability == ABSTRACT and ilk->mParent);
        if (objlore) {
            if (kObjGenericKeycard != ilk->mId) lore->addPageBreak ();
            addEntry (lore, objlore);
        }
    } while (objlore);
    lore->finish ();
...
shCreature::showLore ()
{
    ...
    if (ilk->mNumAppearingDice != 1 or ilk->mNumAppearingDieSides != 1) {
        lore->addText ("");
        snprintf (buf, SHBUFLEN, "Usually appears in groups of %d-%d.",
            ilk->mNumAppearingDice,
            ilk->mNumAppearingDice * ilk->mNumAppearingDieSides);
        lore->addText (buf);
    }
    lore->addPageBreak ();
    /* Next, lore and fluff. */
    ...
    if (Hero.is (kXenosHunter) and isXenos ()) {
        monlore = findLoreEntry ("xenos scum", 1);
        for (int l = 0; l < monlore->numlines; ++l) {
            lore->addText (monlore->lines[l]);
        }
        lore->addPageBreak ();
    }
    monlore = findLoreEntry (myIlk ()->mName, 1);
    if (!monlore) {
