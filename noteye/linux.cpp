...
//      else if(n == 6) originmode = c == 'h';
        else ldf && fprintf(ldf, "{NOTEYE %d%c}", n, c);
        }
      st = sNormal;
      // todo?
      }
...
void LinuxProcess::sendKey(int symbol, int mod, bool down, int unicode) {
  if(!down) return;
...
bool LinuxProcess::checkEvent(lua_State *L) {
  ...
  if(ret) {
    ...
    isActive = false;
    lua_newtable(L);
    setfield(L, "type", evProcQuit);
    setfield(L, "obj", id);
    setfield(L, "exitcode", WEXITSTATUS(exitcode));
    return true;
    }

  if(changed) {
    lua_newtable(L);
    setfield(L, "type", evProcScreen);
    setfield(L, "obj", id);
    return true;
    }
