...
int lh_loadsound(lua_State *L) {
  ...
  initAudio();
  if(audio == 1) return retInt(L, 0);
  Mix_Chunk *chk = Mix_LoadWAV(luaStr(1));
  if(!chk) return retInt(L, 0);
  Sound *snd = new Sound();
  snd->chunk = chk;
  return retObject(L, snd);
  }
...
int lh_playsound(lua_State *L) {
  ...
  return retInt(L, s->play(luaInt(2)));
  }
...
int lh_playsoundloop(lua_State *L) {
  ...
  if(audio == 1 || luaInt(1) == 0) return 0;
  Sound *s = luaO(1, Sound);
  return retInt(L, s->play(luaInt(2), luaInt(3)));
  }
