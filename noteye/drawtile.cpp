...
void blitImage(Image *dest, int x, int y, TileImage *TI) {
  ...
  srcrect.x = TI->ox; srcrect.y = TI->oy;
  srcrect.w = TI->sx; srcrect.h = TI->sy;
  SDL_Rect destrect;
...
//  printf("trans = %08x\n", TI->trans);
  ...
  if((abs(M.tx)+abs(M.tyx))*(abs(M.ty)+abs(M.txy)) <= 128*128) {  
    TransCache tc;
    tc.orig = TI;
...
TileImage *getFillCache(TileFill *TF) {
  if(!TF->cache) {
    ...
    int& pix(qpixel(pcache->s, pcachex, 0));
    pix = TF->color;
