...
#ifdef OPENGL  
  if(useGL(&gfx)) {
    renderAffineImageGL(w, orig, ox, oy);
    return;
    }
#endif
...
void drawFPP(int x, int y, int c) {
  ...
  if(y<=0) renderChar(distill(c, spWallN), rot(x*32+16, -y*32+16), rot(x*32-16, -y*32+16), rot(0,-32),  -16, +16);
  if(x<=0) renderChar(distill(c, spWallW), rot(x*32-16, -y*32+16), rot(x*32-16, -y*32-16), rot(32,0),  -16, +16);
  if(y>=0) renderChar(distill(c, spWallS), rot(x*32-16, -y*32-16), rot(x*32+16, -y*32-16), rot(0,32), -16, +16);
  if(x>=0) renderChar(distill(c, spWallE), rot(x*32+16, -y*32-16), rot(x*32+16, -y*32+16), rot(-32,0), -16, +16);
  
  // 12 advance chars  
  V.shiftdown = true;
  cd pos = rot(x*32, -y*32);
  renderChar(distill(c, spCenter), pos + cd(-V.ctrsize, 0), pos + cd(V.ctrsize, 0), cd(0, V.ctrsize), -16, +16);
  renderChar(distill(c, spMonst), pos + cd(-V.monsize, -V.monpush), pos + cd(V.monsize,-V.monpush), cd(0, V.monsize), -16, +16);
  renderChar(distill(c, spItem), pos + cd(-V.objsize, -V.objpush), pos + cd(+V.objsize,-V.objpush), cd(0, V.objsize), -2, +16);
...
void drawFPPat(int wax, int way, int facedir, Screen *s) {
  ...
  cd mxy = cd(128, 128) + facevecc * cd(V.delta.x, -V.delta.y) * cd(8, 0);
  ...
  for(int x=V.x0; x<V.x1; x++) {
    ...
    u *= -facevecc;
    ux = int(floor(real(mxy) + real(u) / V.xz)), uy = int(floor(imag(mxy) + imag(u) / V.yz));
...
#else
    //int flag=-1;
    int C=(y > V.ym ? sfloor : sceil).get(wax+ux0,way+uy0);
    #endif
...
void drawFPP_GL(int xx, int yy, int c) {
  if(!c) return;
  ...
  renderAffine(distill(c, spFree), orig, ox, oy, oz);
  renderAffine(distill(c, spFloor), orig, ox, oy, oz);

  orig = rotgl(32*xx-16, 16-yy*32, -16, 0, 0) + V.delta;
  
  if(V.side == 1 || V.side == 2) V.side = 3-V.side;
  renderAffine(distill(c, spCeil), orig, ox, oy, oz);
  if(V.side == 1 || V.side == 2) V.side = 3-V.side;
  ...
  if(yy <= 0) {
    ...
    oy = rotgl(0, 0, 32, 0, 0);
    oz = rotgl(0, -32, 0, 0, 0);
    renderAffine(distill(c, spWallN), orig, ox, oy, oz);
    }
  ...
  if(xx <= 0) {
    ...
    oy = rotgl(0, 0, 32, 0, 0);
    oz = rotgl(32, 0, 0, 0, 0);
    renderAffine(distill(c, spWallW), orig, ox, oy, oz);
    }
  ...
  if(yy >= 0) {
    ...
    oy = rotgl(0, 0, 32, 0, 0);
    oz = rotgl(0, 32, 0, 0, 0);
    renderAffine(distill(c, spWallS), orig, ox, oy, oz);
    }
  ...
  if(xx >= 0) {
    ...
    oy = rotgl(0, 0, 32, 0, 0);
    oz = rotgl(-32, 0, 0, 0, 0);
    renderAffine(distill(c, spWallE), orig, ox, oy, oz);
    }
  ...
  oy = rotgl(0, 0, 32, 0, 0);
  oz = rotgl(0, 0, 0, 0, 16);
  renderAffine(distill(c, spCenter), orig, ox, oy, oz);

  orig = rotgl(32*xx, -yy*32, -16, -16, V.monpush) + V.delta;
  ox = rotgl(0, 0, 0, 32, 0);
  oy = rotgl(0, 0, 32, 0, 0);
  oz = rotgl(0, 0, 0, 0, 32);
  renderAffine(distill(c, spMonst), orig, ox, oy, oz);

  orig = rotgl(32*xx, -yy*32, 0, -8, V.objpush) + V.delta;
  ox = rotgl(0, 0, 0, 16, 0);
  oy = rotgl(0, 0, 16, 0, 0);
  oz = rotgl(0, 0, 0, 0, 16);
  renderAffine(distill(c, spItem), orig, ox, oy, oz);
...
void drawFPP_GL(int wax, int way, int facedir, Screen *s) {
  ...
  for(int i=0; i<fpporder.size(); i++) {
    int X = fpporder[i].x;
    int Y = fpporder[i].y;
    drawFPP_GL(X, Y, s->get(wax+X,way+Y));
    }
  }
...
int lh_fpp(lua_State *L) {
  ...
  viewimage = byId<Image> (getfield(L, "vimg"), L);
  viewimg = viewimage->s;
  ...
  V.ctrsize = getfield(L, "ctrsize", 16);
  V.monsize = getfield(L, "monsize", 16);
  V.objsize = getfield(L, "objsize", 8);
  V.monpush = getfield(L, "monpush", 12);
  V.objpush = getfield(L, "objpush", 14);
  ...
  V.side = getfield(L, "side", 1);

#ifdef OPENGL  
  if(useGL(viewimage)) {
    initFPPGL();
    drawFPP_GL(luaInt(2), luaInt(3), luaInt(4), luaO(5, Screen));
    initOrthoGL();
