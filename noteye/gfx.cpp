...
int lh_setvideomode(lua_State *L) {
  ...
  return retBool(L, setvideomode(sx, sy, (full ? SDL_FULLSCREEN : 0)));
  }

int lh_setvideomodex(lua_State *L) {
  checkArg(L, 3, "setvideomodex");

  return retBool(L, setvideomode(luaInt(1), luaInt(2), luaInt(3)));
  }
...
bool GFX::checkEvent(lua_State *L) {
  ...
  if(SDL_PollEvent(&ev)) {
    if(ev.type == SDL_KEYDOWN || ev.type == SDL_KEYUP) {
      ...
      lua_newtable(L);
      setfield(L, "type", ev.type == SDL_KEYDOWN ? evKeyDown : evKeyUp);
      setfield(L, "symbol", kev.keysym.sym);
      setfield(L, "mod", kev.keysym.mod);
      setfield(L, "chr", kev.keysym.unicode);
      return true;
      }
    ...
    if(ev.type == SDL_MOUSEMOTION) {
      ...
      lua_newtable(L);
      setfield(L, "type", evMouseMotion);
      setfield(L, "x", mev.x);
      setfield(L, "y", mev.y);
      setfield(L, "state", mev.state);
      return true;
      }
    ...
    if(ev.type == SDL_MOUSEBUTTONDOWN || ev.type == SDL_MOUSEBUTTONUP) {
      ...
      lua_newtable(L);
      setfield(L, "type", ev.type == SDL_MOUSEBUTTONDOWN ? evMouseDown : evMouseUp);
      setfield(L, "x", mev.x);
      setfield(L, "y", mev.y);
      setfield(L, "state", SDL_GetMouseState(NULL, NULL));
      setfield(L, "button", mev.button);
      return true;
      }

    if(ev.type == SDL_QUIT) {
      lua_newtable(L);
      setfield(L, "type", evQuit);
      return true;
      }
    }
  return false;
