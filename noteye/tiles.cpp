...
int lh_getdistill(lua_State *L) {
  ...
  return retInt(L, distill(luaInt(1), luaInt(2)));
  }
...
int getFppDown(TileImage *T) {
  if(T->fppdown != -1) return T->fppdown;
  int ay;
  if(T->trans == transAlpha) {
    for(ay=T->sy-1; ay >= 0; ay--) for(int ax=0; ax<T->sx; ax++)
      if(part(qpixel(T->i->s, T->ox+ax, T->oy+ay), 3)) goto ayfound;
    }
  else {
    for(ay=T->sy-1; ay >= 0; ay--) for(int ax=0; ax<T->sx; ax++)
      if(!istrans(qpixel(T->i->s, T->ox+ax, T->oy+ay), T->trans)) goto ayfound;
    }
  T->fppdown = 0;
  //printf("%d (%s): no down\n", T->id, T->i->title.c_str());
  return 0;
  ayfound: T->fppdown = T->sy - ay - 1;
  //printf("%d (%s): %d down\n", T->id, T->i->title.c_str(), T->fppdown);
  return T->fppdown;
  }
...
int lh_getobjectinfo(lua_State *L) {
  ...
  Get(TileImage, TI, id);
  if(TI) {
    setfield(L, "type", 0x11);
    setfield(L, "ox", TI->ox);
    setfield(L, "oy", TI->oy);
    setfield(L, "sx", TI->sx);
    setfield(L, "sy", TI->sy);
    setfield(L, "ch", TI->chid);
    setfield(L, "trans", TI->trans);
    setfield(L, "i", TI->i->id);
    setfield(L, "bottom", getFppDown(TI));
    return 1;
    }

  Get(TileRecolor, TR, id);
  if(TR) {
    setfield(L, "type", 0x21);
    setfield(L, "t1", TR->t1);
    setfield(L, "mode", TR->mode);
    setfield(L, "color", TR->color);
    return 1;
    }
  
  Get(TileMerge, TM, id);
  if(TM) {
    setfield(L, "type", TM->over ? 0x18 : 0x12);
    setfield(L, "t1", TM->t1);
    setfield(L, "t2", TM->t2);
    return 1;
    }
  ...
  if(TSp) {
    setfield(L, "type", 0x14);
    setfield(L, "t1", TSp->t1);
    setfield(L, "sf", TSp->sf);
    return 1;
    }
  
  Get(TileLayer, TL, id);
  if(TL) {
    setfield(L, "type", 0x19);
    setfield(L, "t1", TL->t1);
    setfield(L, "sf", TL->layerid);
    return 1;
    }
  
  Get(TileFill, TF, id);
  if(TF) {
    setfield(L, "type", 0x20);
    setfield(L, "color", TF->color);
    setfield(L, "alpha", TF->alpha);
    return 1;
    }
  
  Get(Tile, TX, id);
  if(TX) {
    setfield(L, "type", 0x10);
    return 1;
    }
