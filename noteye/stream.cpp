...
void NStream::writeChar(char c) {
  ...
  if(zout.avail_in == BUFSIZE) {
    while(zout.avail_in || !zout.avail_out) {
      zout.avail_out = BUFSIZE;
      zout.next_out = outbuf_out;
      deflate(&zout, 0);
      for(int i=0; i<int(BUFSIZE-zout.avail_out); i++) writeCharPrim(outbuf_out[i]), writeCmp++;
      }
    zout.next_in = outbuf_in;
    }
...
bool NStream::ready() {
  proceed(true);
  return finished || zin.avail_out < BUFSIZE;
  }

void NStream::proceed(bool check) {
  if(finished) return;
  if(!inok) {
    zin.avail_in = 0;
...
int lh_writefile(lua_State *L) {
  ...
  if(!S->f) { delete S; return retInt(L, 0); }
  return retObject(L, S);
  }
...
int lh_flush(lua_State *L) {
  checkArg(L, 1, "nflush");
  luaO(1, NStream)->flush();
  return 0;
  }

int lh_finish(lua_State *L) {
  checkArg(L, 1, "nfinish");
  luaO(1, NStream)->finish();
  return 0;
  }

int lh_ready(lua_State *L) {
  checkArg(L, 1, "nready");
  lua_pushboolean(L, luaO(1, NStream)->ready());
  return 1;
  }
#endif
...
int lh_server(lua_State *L) {
  ...
  return retObject(L, new TCPServer(skt));
  }
...
int lh_accept(lua_State *L) {
  ...
  if(!skt) return retInt(L, 0);
  return retObject(L, new NTCPStream(skt));
  }
